 ...
 * @returns The kind of hand represented by the 5
 */
const getHand = (cards: string): Hand => {
  const cardDistribution: object = cards.split('').reduce((acc, el) => {
    if (el in acc) {
      acc[el] = acc[el] + 1;
    } else {
      acc[el] = 1;
  ...
  const distributionValues = Object.values(cardDistribution);
